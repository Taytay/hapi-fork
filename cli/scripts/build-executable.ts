...
import { trimIdent } from '../src/utils/trimIdent';

const DEFAULT_TARGETS = ['bun-darwin-x64', 'bun-darwin-arm64', 'bun-linux-x64', 'bun-linux-arm64', 'bun-windows-x64'];
...
async function main(): Promise<void> {
    ...
    if (args.includes('--target') && !target) {
        console.error(
            'Usage: bun run scripts/build-executable.ts [--target <bun-platform[-arch]>] [--outdir dist-exe] [--name hapi] [--with-web-assets]',
        );
        console.error(
            '   or: bun run scripts/build-executable.ts --all [--outdir dist-exe] [--name hapi] [--with-web-assets]',
        );
        process.exit(1);
    }
