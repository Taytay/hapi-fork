...
import type { Metadata, RunnerState } from '@/api/types';
import type { SpawnSessionOptions, SpawnSessionResult } from '@/modules/common/rpcTypes';
import {
    acquireRunnerLock,
    type RunnerLocallyPersistedState,
    readRunnerState,
    releaseRunnerLock,
    writeRunnerState,
} from '@/persistence';
...
export async function startRunner(): Promise<void> {
    ...
    const resolvesWhenShutdownRequested = new Promise<{
        source: 'hapi-app' | 'hapi-cli' | 'os-signal' | 'exception';
        errorMessage?: string;
    }>((resolve) => {
    ...
    try {
        ...
        const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
            ...
            if (sessionType === 'worktree') {
                ...
                logger.debug(
                    `[RUNNER RUN] Created worktree ${worktreeInfo.worktreePath} (branch ${worktreeInfo.branch})`,
                );
            }
            ...
            const cleanupWorktree = async () => {
                ...
                if (!result.ok) {
                    logger.debug(
                        `[RUNNER RUN] Failed to remove worktree ${worktreeInfo.worktreePath}: ${result.error}`,
                    );
                }
            };
            ...
            try {
                ...
                const spawnResult = await new Promise<SpawnSessionResult>((resolve) => {
                    ...
                    pidToAwaiter.set(pid, (completedSession) => {
                        ...
                        logger.debug(
                            `[RUNNER RUN] Session ${completedSession.happySessionId} fully spawned with webhook`,
                        );
                        resolve({
                            type: 'success',
        ...
        const stopSession = (sessionId: string): boolean => {
            ...
            for (const [pid, session] of pidToTrackedSession.entries()) {
                if (
                    ...
                    (sessionId.startsWith('PID-') && pid === parseInt(sessionId.replace('PID-', ''), 10))
                ) {
        ...
        // 4. Write heartbeat
        const heartbeatIntervalMs = parseInt(process.env.HAPI_RUNNER_HEARTBEAT_INTERVAL || '60000', 10);
        ...
        const restartOnStaleVersionAndHeartbeat = setInterval(async () => {
            ...
            if (
                ...
                logger.debug(
                    '[RUNNER RUN] Runner is outdated, triggering self-restart with latest version, clearing heartbeat interval',
                );
                ...
                try {
                    ...
                    logger.debug(
                        '[RUNNER RUN] Failed to spawn new runner, this is quite likely to happen during integration tests as we are cleaning out dist/ directory',
                        error,
                    );
                }
                ...
                logger.debug(
                    '[RUNNER RUN] Hanging for a bit - waiting for CLI to kill us because we are running outdated version of the code',
                );
            ...
            if (runnerState && runnerState.pid !== process.pid) {
                logger.debug(
                    '[RUNNER RUN] Somehow a different runner was started without killing us. We should kill ourselves.',
                );
                requestShutdown(
                    'exception',
