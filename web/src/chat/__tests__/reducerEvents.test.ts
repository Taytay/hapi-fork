...
import { parseMessageAsEvent, dedupeAgentEvents, foldApiErrorEvents } from '@/chat/reducerEvents'
import type { NormalizedMessage, ChatBlock, AgentEventBlock } from '@/chat/types'
...
describe('parseMessageAsEvent', () => {
    it('parses Claude usage limit text into limit-reached event', () => {
        const msg: NormalizedMessage = {
            ...
            role: 'agent',
            isSidechain: false,
            content: [{ type: 'text', text: 'Claude AI usage limit reached|1700000000', uuid: 'u1', parentUUID: null }],
        }
    ...
    it('returns null for user messages', () => {
        const msg: NormalizedMessage = {
            ...
            role: 'user',
            isSidechain: false,
            content: { type: 'text', text: 'Claude AI usage limit reached|1700000000' },
        }
        expect(parseMessageAsEvent(msg)).toBeNull()
    })
})
...
describe('dedupeAgentEvents', () => {
    function makeEvent(id: string, event: { type: string; [key: string]: unknown }): AgentEventBlock {
        return { kind: 'agent-event', id, createdAt: 1000, event: event as AgentEventBlock['event'] }
    }
    ...
    it('resets dedup tracking when non-event block appears', () => {
        const blocks: ChatBlock[] = [
            ...
            { kind: 'agent-text', id: 'a1', localId: null, createdAt: 1000, text: 'text' },
...
describe('foldApiErrorEvents', () => {
    ...
    it('folds consecutive api-error events keeping only the latest', () => {
        const blocks: ChatBlock[] = [
            makeEvent('e1', { type: 'api-error', retryAttempt: 1, maxRetries: 3, error: 'err1' }),
            makeEvent('e2', { type: 'api-error', retryAttempt: 2, maxRetries: 3, error: 'err2' }),
            makeEvent('e3', { type: 'api-error', retryAttempt: 3, maxRetries: 3, error: 'err3' }),
        ]
