...
describe('isCliOutputText', () => {
    it('returns true for text with CLI tags and meta.sentFrom === "cli"', () => {
        ...
        expect(
            isCliOutputText('<command-message>done</command-message>', {
                sentFrom: 'cli',
            }),
        ).toBe(true);
        expect(isCliOutputText('<command-args>-la</command-args>', { sentFrom: 'cli' })).toBe(true);
        expect(
            isCliOutputText('<local-command-stdout>output</local-command-stdout>', {
                sentFrom: 'cli',
            }),
        ).toBe(true);
    });
    ...
    it('handles local-command-* tags', () => {
        expect(
            isCliOutputText('<local-command-stderr>error</local-command-stderr>', {
                sentFrom: 'cli',
            }),
        ).toBe(true);
    });
