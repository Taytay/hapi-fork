...
import { normalizeAgentRecord, isSkippableAgentContent, isCodexContent } from '@/chat/normalizeAgent'
...
describe('normalizeAgentRecord', () => {
    describe('assistant output', () => {
        it('normalizes a simple text assistant message', () => {
            ...
            const block = (result!.content as unknown[])[0] as { type: string; text: string }
        ...
        it('extracts tool description from input.description', () => {
                        ...
                        content: [
                            {
                                ...
                                id: 'tool-2',
                                name: 'Write',
                                input: { description: 'Write a file', file: 'out.ts', content: '...' },
                            },
                        ],
                    },
                },
            ...
            const block = (result!.content as unknown[])[0] as { description: string | null }
    ...
    describe('system events', () => {
        it('normalizes api_error event', () => {
            ...
            const event = result!.content as { type: string; retryAttempt: number; maxRetries: number }
        ...
        it('normalizes microcompact_boundary event', () => {
            ...
            expect(result).not.toBeNull()
            const event = result!.content as { type: string; trigger: string; preTokens: number; tokensSaved: number }
        ...
        it('normalizes compact_boundary event', () => {
            ...
            expect(result).not.toBeNull()
            const event = result!.content as { type: string; trigger: string; preTokens: number }
    ...
    describe('codex content', () => {
        ...
        it('normalizes codex tool-call', () => {
            const content = {
                type: 'codex',
                data: { type: 'tool-call', callId: 'call-1', name: 'Bash', input: { command: 'ls' } },
            }
        ...
        it('normalizes codex tool-call-result', () => {
            const content = {
                type: 'codex',
                data: { type: 'tool-call-result', callId: 'call-1', output: 'file1\nfile2' },
            }
    ...
    describe('edge cases', () => {
        ...
        it('skips meta and compact-summary messages', () => {
            ...
            const compact = {
                type: 'output',
                data: { type: 'assistant', isCompactSummary: true, message: { content: '' } },
            }
...
describe('isSkippableAgentContent', () => {
    ...
    it('returns true for isCompactSummary output', () => {
        expect(isSkippableAgentContent({ type: 'output', data: { isCompactSummary: true } })).toBe(true)
    })
