...
describe('traceMessages', () => {
    it('returns messages as TracedMessages without sidechainId for non-sidechain messages', () => {
        const messages: NormalizedMessage[] = [
            ...
            makeAgentMessage({ id: 'a1', content: [{ type: 'text', text: 'hi', uuid: 'uuid1', parentUUID: null }] }),
        ]
    ...
    it('propagates sidechainId via parentUUID chain', () => {
        ...
        const messages: NormalizedMessage[] = [
            ...
            makeAgentMessage({
                id: 'sc-child',
                isSidechain: true,
                content: [{ type: 'text', text: 'found it', uuid: 'uuid-sc-child', parentUUID: 'uuid-sc-root' }],
            }),
        ]
        const result = traceMessages(messages)
    ...
    it('handles orphan messages — resolves once parent becomes available', () => {
        ...
        const messages: NormalizedMessage[] = [
            ...
            makeAgentMessage({
                id: 'orphan',
                isSidechain: true,
                content: [{ type: 'text', text: 'orphan text', uuid: 'uuid-orphan', parentUUID: 'uuid-sc-root' }],
            }),
